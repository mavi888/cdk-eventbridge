...
export INVENTORY_TABLE=$(aws cloudformation describe-stacks --stack-name $1 \
  --query 'Stacks[0].Outputs[?OutputKey==`InventoryTableOutput`].OutputValue' --output text) 
...
export CUSTOMER_TABLE=$(aws cloudformation describe-stacks --stack-name $1 \
  --query 'Stacks[0].Outputs[?OutputKey==`CustomerTableOutput`].OutputValue' --output text) 
